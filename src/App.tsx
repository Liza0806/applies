...
const getFilteredWorks = () => {
  return works.filter((work) => {
    ...
    const matchStatus = applicationStatus ? work.status === applicationStatus : true;
    const matchFormat = workFormat ? work.workFormat === workFormat : true;
    const matchEmployment = employmentType ? work.employmentType === employmentType : true;
    const matchTestTask = testTask
      ? work.testTask?.required === testTask.required
      : true;

    return matchTrust && matchStatus && matchFormat && matchEmployment && matchTestTask;
  });
};
const resetFilters = () => {
  setTrustLevel("");
...
<select value={applicationStatus} onChange={(e) => setApplicationStatus(e.target.value as ApplicationStatus | "")}>
  ...
  <select value={workFormat} onChange={(e) => setWorkFormat(e.target.value as WorkFormat | "")}>
...
<select
  value={testTask?.required === true ? "true" : testTask?.required === false ? "false" : ""}
  onChange={(e) => {
